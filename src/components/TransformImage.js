...
import { Effect, RoundCorners, Rotate, Resize, Overlay } from '@cloudinary/url-gen/actions';
...
const TransformImage = ({ imageKey }) => {
	...
	const getURLofS3Object = (tag) => {
		const S3BucketName = 'myownstorage12324335-dev';
	...
	const getImage = async () => {
		...
		image.resize(Resize.fill().width(340))
			.effect(Effect.artisticFilter('frost'))
			.roundCorners(RoundCorners.byRadius().radius(40))
			.rotate(Rotate.byAngle(20))
			.overlay(
				Overlay.source(
					Source.text(imageKey.replace(/\.\w+$/, ''), new TextStyle('Cookie', 50).fontWeight(800))
						.textColor('#0C0C14')
						.transformation(new Transformation().effect(Effect.shadow(4).color('#FFCC43'))),
